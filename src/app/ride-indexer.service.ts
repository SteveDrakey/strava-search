...
export class RideIndexerService {
  ...
  async index(page: number): Promise<number> {
    ...
    for (const activitySummary of activities.slice(1, 100)) {
      try {
        ...
        try {
          const doc =  await this.client.getDocument(activitySummary.id.toString());
          // console.log( new Date(activitySummary.start_date).toLocaleDateString());
          this.OnIndexedSubject$.next(`Date: ${new Date(activitySummary.start_date).toLocaleDateString()} , Title: ${activitySummary.name}`);
  ...
  async find(term, segments?: string[], city?: string ): Promise<Activity[]> {

    console.log('a', this.authentication.currentAthlete);
      ...
      await this.client.search( term ? term.replace(/(\w+)/g, '$1~1')  : null ,
        {
          ...
          top: 50,
          filter:  [
            `athlete eq ${this.authentication.currentAthlete.id}` ,
             segmentsFilter,
             cityFilter
            ].filter(f => f).join(' and '),
          facets: ['segments,count:20', 'cities,count:20' ],
    ...
    for await (const result of searchResults.results) {
      rval.push(
        { activity: result.id, title: result.title, description: result.description, highlights: result.highlights,
          when: result.when, score: result.score });
    }
    console.log('searchResults', searchResults);
    return rval;
